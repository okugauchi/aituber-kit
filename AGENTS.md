...
- 設定画面の単独フォームはモバイルで`w-full`とし、`sm`以上では入力内容に応じて幅を制限する
- 横幅は`src/components/settings/formStyles.ts`の共通定義を使用し、画面ごとに任意の幅を追加しない
- `compact`（184px）: 言語、表示位置、短い選択肢、短い名前
- `medium`（392px）: モデル、話者、ファイル・フォルダ名、IDなどの中程度の文字列
- `long`（704px）: APIキー、URL、エンドポイントなどの長い文字列
- `full`: 文章、プロンプト、テキストエリア、検索欄、スライダー
- `numeric`（96px）/`numericWide`（128px）: 数値入力。桁数に応じて使い分け、パスコードなどは必要に応じて`w-48`までとする
- 複合行では主入力を`flex-1`、補助選択肢や単位入力を固定幅にしてよい。狭い画面で窮屈になる場合は縦積みに切り替える
- 操作ボタンはPCでは内容幅を基本とし、モバイルで必要な場合のみ`w-full sm:w-auto`を使用する
- 追加・変更時は、条件分岐で表示されるフォームも含めてPC幅とモバイル幅を確認する
